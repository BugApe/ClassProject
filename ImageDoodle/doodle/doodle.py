...
def parse_args(input,style):
    ...
    add_arg('--style', default='doodle/samples/'+style, type=str, help='Style image path to extract patches.')
...
class NeuralGenerator(object):
    ...
    def load_images(self, name, filename):
        ...
        img =imageio.imread(filename, pilmode='RGB') if os.path.exists(filename) else None
        map = imageio.imread(mapname) if os.path.exists(mapname) and args.semantic_weight > 0.0 else None
    ...
    def rescale_image(self, img, scale):
        ...
        """
        output = PIL.Image.fromarray(img, mode='RGB')
